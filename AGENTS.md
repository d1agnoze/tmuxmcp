...
- Pane previews are captured directly by the client with tmux commands, preserving ANSI styling; the popup does not ask the daemon for previews.
...
- The popup supports focus switching between the table and preview; mouse wheel scrolling is routed to the preview.
...
- For end-to-end validation, run `tmuxmcpd`, open `tmuxmcp` inside `tmux popup`, verify table navigation, preview updates, pane select/unshare behavior, then hit `GET /healthz`, `GET/POST/DELETE /active-pane`, and finally verify `get_active_pane` and `read_active_pane` from an MCP host/client.
